...
const SeedCreation = ({handleSteps}:any) => {
    ...
    const submitMnemonic = () =>{
        for (let index = 0; index < randomNumberList.length; index++) {
            let phrase = document.getElementById('mnemonicKey' + randomNumberList[index]) as HTMLInputElement;
    ...
    return (
        <>
            <div className="px-8 pt-8 md:px-6 md:pt-6">
                <button className="absolute left-[50px] top-[40px]" onClick={() => handleSteps("1")}>
                    <Icon
                        viewClass="arrow-right"
                        iconName="left-arrow"/>
                </button>
                <p className="text-center text-light-high font-semibold text-2xl leading-normal">Create Wallet</p>
            </div>
            <div className="p-6">
                <div className="mb-2 text-center flex
                justify-center items-center relative">
                    <span className="mr-2 ext-light-emphasis font-bold text-base leading-normaL">Mnemonic (Seed Phrase)</span>
                <Copy id={mnemonic}/>
                </div>
                    <div className="flex flex-wrap justify-center mb-4">
                        {quiz ?
                           <>
                               <p className={"mb-4 max-w-[80%] mx-auto text-center"}>Enter your mnemonics below
                                   to make sure you saved mnemonics correctly</p>
                               <div className="flex flex-wrap justify-center">
                               {randomNumberList.length && randomNumberList.map((number: any, index: null) => (
